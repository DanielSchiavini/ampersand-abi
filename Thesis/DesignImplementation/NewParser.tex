...
\begin{description}
  \item[Classify]
    ...
    When the parser encounters \code{`CLASSIFY'}, it cannot define whether it found a \code{Classify} or a \code{GenDef} production.
  ...
  \item[Role]
    ...
    When the parser encounters \code{`ROLE'}, it cannot define whether it is a \code{RoleRelation} or a \code{RoleRule} production.
  ...
  \item[View]
    ...
    That's because the productions \code{Label} and \code{LabelProps} are not disjoint, and \code{`DEFAULT'} is optional.
  ...
  \item[Labels and Terms]
    In the productions \code{Att} and \code{RuleDef}, we see very similar ambiguities:
    \begin{ebnf}
     Att ::= LabelProps? Term
    ...
    And one of the possible productions of \code{Term} is:
    \begin{ebnf}
