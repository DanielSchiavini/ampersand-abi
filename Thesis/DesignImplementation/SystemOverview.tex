...
\label{design:system-overview}
  ...
  Once the input string is separated into the token structure as defined in the module \code{LexerToken} the next step is to actually parse the tokens by the \{Parser}.
  ...
  \begin{description}
    ...
    \item[Lexer/LexerToken] modules responsible for recognizing the input characters and converting them to tokens.
      ...
      This list of tokens if the actual input for the parser.
 ...
 \end{description}
  ...
  Several supporting modules are defined used by one or more main moules:
  \begin{description}
