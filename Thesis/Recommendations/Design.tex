...
\begin{description}
  \item[Syntax improvements]
    ...
    The Parsec \code{try} function allows the backtracking in the parser, avoiding that input is consumed which is still needed in another parse statements if the parse function can not succeed successfully.
 ...
 data P_ObjDef a =
    ...
    As the order of the parameters in the parse tree and the parser are different, the intermediate function \code{rebuild} is used to align both types to each other.
  ...
  \item[Manual overrule of error message]
    ...
    If the Ampersand teams want to tweak some error message after all, this can be realized using the \code{<?>} Parsec function.
    Placing the \code{<?>} after the parser, followed by a string, changes the standard Parsec text after the keyword `expecting'.
