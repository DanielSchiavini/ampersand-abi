...
\begin{description}
  ...
  \item[Content]
    ...
    As you see, the \texttt{RecordList} production starts with a \texttt{String}, while the \texttt{RecordObsList} starts with a `('.
    TODO: Why do we need try here?
  ...
  \item[Multiplicity]
    ...
    \begin{quote}
        \texttt{Mult ::= (`0' | `1') `..' (`1' | `*') | `*' | `1'}
    \end{quote}
    When the parser encounters \texttt{`1'}, it cannot define whether it found the first or the last option.
    The parser must therefore read the next token before choosing the production.
    ...
    \end{quote}
    %
    We did not refactor the code in this matter because the \texttt{pMult} parser does more than only parsing: it also changes the representation of the found constructions before saving the parse tree.
  
  \item[Labels and Terms]
    In the productions \texttt{Index}, \texttt{ViewAtt} and \texttt{RuleDef}, we see very similar ambiguities:
    \begin{quote}
        \texttt{Index ::= `IDENT' Label ConceptRefPos `(' IndSegmentList ')'}\\
        \texttt{ViewAtt ::= LabelProps? Term}\\
        \texttt{RuleDef ::= `RULE' (ADLid `:')? Rule Meaning* Message* Violation?}\\
        wherein:
        \texttt{Label ::= ADLid ':'}\\
    ...
    Since the \texttt{Term} grammar is quite complex, this issue may severely impact the parser's performance.
    ...
    TODO: (ADLid `:')? should be Label? in RuleDef\\
    TODO: RelSign doesn't exist. Should be NamedRel\\
\end{description}
%
